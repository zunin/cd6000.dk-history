...
--
-- The 2026-05-31 placeholder is replaced by the CI workflow with the current date (YYYY-MM-DD).
--
-- Usage:
--   sed "s/2026-05-31/$(date -u +%Y-%m-%d)/g" freeze.sql | duckdb
...
) TO 'parquet/cds-2026-05-31.parquet';

-- Step 2: Attach the DuckLake catalog (creates the file if it doesn't exist)
ATTACH 'ducklake:cd6000.ducklake' AS lake (DATA_PATH 'parquet/');
